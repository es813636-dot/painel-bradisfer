...
- **Margem líquida real (2026-08-18)** — importada de um export do Sysemp ("manutenção da tabela de preços", CSV com 4.462 produtos) e embutida como `TABELA_PRECOS` no `index.html`, casada por `codigoBarras` (~97% dos produtos batem). Aparece na "popup lateral" (modal de itens mais vendidos de uma marca), como margem líquida + preço de venda por item. **É uma foto estática do dia da exportação** — preço/margem mudam com reajuste e isso não atualiza sozinho; pra manter vivo, o ideal é subir numa aba nova da mesma planilha Google (mesmo padrão de `BaseLooker`/`AnaliseMinMax`) em vez de reexportar/colar no código toda vez. Ainda não fiz essa migração.
- **Sem preço de venda no resto do painel** — fora da margem líquida acima, o resto do painel só tem custo de compra (`custo`). Onde já existe uma estimativa de faturamento (aba Vendas), continua sinalizada como estimativa, calculada com custo × média vendida.
