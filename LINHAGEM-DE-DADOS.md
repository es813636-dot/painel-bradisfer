...
| `listarVendasPorVendedor` | `automacao-vendas/atualizar-vendas-online.js` (empresas 3/4 — marketplace) e `automacao-vendas/atualizar-vendas-bradisfer.js` (empresa 1 — B2B) | A cada 5 min (incremental por checkpoint — pedido era 3 min, piso real do GitHub Actions é 5). **Bradisfer com `schedule` desligado por ora, só `workflow_dispatch`** — ver CONTEXTO.md | Vendas por vendedor/canal/período/data de emissão/`id_pedido` (só na versão Bradisfer) — ver `CONTEXTO.md` |
...
| **VendasOnlineControle** | Escrita direta (`values.clear` + `values.update`) por `atualizar-vendas-online.js` — 2 linhas, `IdEmpresa`/`UltimaDataEmissaoProcessada` | Automação (GitHub Actions, a cada 5 min, junto com `VendasOnline`) | Fresca | Só o próprio `atualizar-vendas-online.js` (checkpoint de onde continuar) — não é dado de negócio, é estado técnico do script |
| **VendasBradisfer** | Escrita direta (`values.append`, dedup por `ChaveDedup` reforçada com `id_pedido`) por `atualizar-vendas-bradisfer.js` (03/09/2026) — aba criada automaticamente na 1ª execução | Automação (GitHub Actions, só `workflow_dispatch` por ora — `schedule` desligado até validar volume) | Fresca a partir da 1ª carga histórica (desde 2026-01-01) | **Nenhum consumidor no painel HTML.** Alimenta o projeto Power BI separado `C:\Users\Admin\Documents\Dados BI` (substitui `Fact_Vendas`/`Fact_Pedidos`, hoje Excel manual — ver `CLAUDE.md` desse projeto); `script.js` nunca lê essa aba |
| **VendasBradisferControle** | Escrita direta por `atualizar-vendas-bradisfer.js` — 1 linha, `IdEmpresa`/`UltimaDataEmissaoProcessada` | Automação (GitHub Actions, junto com `VendasBradisfer`) | Fresca | Só o próprio `atualizar-vendas-bradisfer.js` — estado técnico, não dado de negócio |
